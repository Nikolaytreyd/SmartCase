...
class Load:
    def __init__(self, variable_file: str):
        self.config_variable_file = Load.resource_path(r"ischod/conditions_variable.json")
        self.default_variable_file = Load.resource_path(r"ischod/by_default_variables.json")
        self.variable_file = Load.check_file(variable_file)
    ...
    """  Получение/Замена значений переменных """
    ...
    def get_variable(self, key: str):
        ...
        if key in self.variable:
            if isinstance(key, str):
                return Load.convert_value_from_str(self.variable[key], self.config_variable[key]['type'])
            else:
    ...
    def set_variable(self, key: str, value):
        ...
        if key in self.variable:
            if isinstance(key, str):
                self.variable[key] = Load.convert_value_to_str(value, self.config_variable[key]['type'])
                self.save_variable()
            else:
                raise AssertionError("ключ {key} к переменной должен задаваться через ''")
        else:
    ...
    def reset_to_default(self):
        ...
        """
        shutil.copyfile(self.default_variable_file, self.variable_file)
        self.load_variable()
...
'''
test = Load()
test.get_variable('duration')
test.help_variable('duration')
test.help_field('description')
'''